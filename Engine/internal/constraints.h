...
// rounds (pins before ropes in each round; the Atwood system is exact after
// one round because the pin and a single rope touch disjoint variables).
...
// Fully removes constraint position errors: pins snap their circle center
// back to the anchor; ropes remove the segment-length-sum error along the
// current segment directions, distributed by inverse mass. Both are full
// snaps (no correction factor, no slop) so drift does not accumulate.
