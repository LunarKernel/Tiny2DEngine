...
shell::SetupAction DrawSetupScreen(ContactLabConfig* config,
...
struct ContactLabTraits {
  using Config = ContactLabConfig;
  using State = ContactLabState;
  static constexpr float kPhysicsStep = kContactLabPhysicsStep;
  static constexpr double kMaximumFrameTime = 0.25;
  static constexpr const char* kInvalidHistoryTimeMessage =
      "ContactLab produced an invalid history time.";
  static constexpr const char* kNonIncreasingHistoryTimeMessage =
      "ContactLab history time stopped increasing.";

  // Captured at Start so the monitor can compare against the launch
  // configuration's derived quantities.
  ContactLabDerived initial_derived{};

  Config MakeInitialConfig() { return MakeElasticImpactConfig(); }
  State MakeState(const Config& config) {
    State state = MakeInitialContactLabState(config);
  ...
  bool Step(const Config& config, State* state) {
    return StepContactLab(config, kPhysicsStep, state);
  }
  std::string StepFailureMessage(const Config& config, const State& state,
                                 std::vector<State>*) {
    const char* error = GetContactLabStateError(config, state);
    return error != nullptr ? error
                            : "ContactLab rejected an unstable fixed step.";
  }
  const char* AfterStepIssue(const State&) { return nullptr; }
  shell::SetupAction DrawSetup(Config* config, const std::string& error) {
    return DrawSetupScreen(config, error);
  }
  bool DrawFrame(const Config& config, const State& state,
    ...
    const bool stop =
        DrawMonitor(config, state, initial_derived, history, paused,
                    inspect_time, follow_live, error, &displayed_state);
...
SimulationResult RunContactLabSimulation(SDL_Renderer* renderer) {
  return shell::RunLab(renderer, ContactLabTraits{});
}
